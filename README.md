...
- **You choose what "popular" means** -- The headline row on the Movies and TV tabs reads TMDB weekly trending by default, or Trakt trending (ranked by who is actually watching), or TMDB's all-time popularity ranking. An optional English-only switch keeps the discovery and recommendation rows to English-language originals; search always finds everything.
...
| Trakt client ID | Admin UI | Enhances discovery + fallback ID bridging; required to select the Trakt trending source under Settings > Discovery |
| Discovery row source | Admin UI | Settings > Discovery: which feed backs the headline rows, plus the optional English-only filter |
