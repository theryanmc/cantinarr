...
- **Audiobookshelf access and listening** -- share an Audiobookshelf server, let people create or link their own accounts, or import existing accounts from Users. Available audiobooks offer **Listen in Audiobookshelf** after a live ISBN/ASIN match within the linked account's library and content access; separate copies stay separate for the listener to choose. **Open Audiobookshelf** is the general shortcut when an exact copy cannot be verified. Removing access disables the account without deleting listening history. See [book setup](docs/books-setup.md#7-optional--listen-with-audiobookshelf).
