...
- **Local AI** -- The included OpenAI provider takes an optional base URL and a pinned reasoning effort, so shared AI can run against a self-hosted OpenAI-compatible server (llama.cpp, vLLM, Ollama) and assistant traffic never leaves your network. The save-time test proves the endpoint before anything is stored, and personal OpenAI keys keep using api.openai.com.
