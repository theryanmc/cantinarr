...
- **Browse before setup** -- Admins start with Movies, TV Shows, Releases, Books, and Music. Explore catalogs and open details before connecting a service. Each unconfigured catalog has a fixed **Set up Radarr/Sonarr/Chaptarr/Lidarr** and **Hide this tab** footer; setup returns to the same catalog or title. Hiding affects everyone on the server, and the tab returns automatically when its service is configured, even if that service is offline. Restore or change these choices under **Settings > Modules > Discover > Discover tabs**. Releases always stays available. Library rows, availability, and book/music search appear once the service is connected; requester and kids access still follows existing grants. Only visible book/music opening rows load in the background.
- **Books too** -- A Chaptarr (Readarr-API) module with per-format smarts: tap a book's eBook or Audiobook row to request that format; monitored formats read **Requested** until they download; a book page links out to its own page on Goodreads and Open Library (and Hardcover when Chaptarr's metadata names it), never a guessed search; owned-aware search and plain per-format controls stay pinned to the selected, authorized Chaptarr instance. Requester access is granted per user. The Books tab starts with **Popular Books** and **Browse by genre** from Open Library, followed by Recently Added, Authors and Series. No extra account or API key is needed. English edition titles and covers are preferred without hiding other books. Popularity follows overall Open Library reading-list counts; discovery finds shared titles and Chaptarr supplies separate eBook and Audiobook request choices. A verified catalog ID connects each discovered work to the existing request flow; an unmatched work offers title-and-author search. Tapping an author or series lists the whole run with each title's per-format state, so a half-finished series shows exactly which books are missing.
- **Music too** -- A Lidarr module: search albums and artists from the Music tab, tap Request on an album, and the artist is added with exactly that album monitored -- one request never subscribes the whole discography. Requester access is granted per user like Books, the tab opens on Popular Albums (this week, month, or year), New Releases from the past 30 days, and genre browsing, followed by Recently Added and Artists from the library. ListenBrainz and MusicBrainz supply albums and EPs automatically, with no extra account or API key, and requests stay pinned to the selected, authorized Lidarr instance. The module carries the rest of the stack too: a push when an album lands, album release dates on the calendar, the Import Doctor and interactive release search for stuck downloads, per-track file downloads, and Report a problem with the same agent repair as books. Full setup in [`docs/music-setup.md`](docs/music-setup.md).
