...
- **Browse before setup** -- Admins see Movies, TV Shows, Releases, Books, and Music from the start. Explore catalogs and open details before connecting a service; a title's request area opens the matching Radarr, Sonarr, Chaptarr, or Lidarr setup form and returns to that title when saved. Library rows, availability, and book/music search appear once the service is connected. Requester and kids access still follows the existing grants. Book and music opening rows start loading in the background when Discovery opens; scrolling keeps the next page and nearby artwork ready.
