...
- **Season cards count the whole season** -- "11/13 Episodes Available • 2 unaired" rather than Sonarr's aired-and-monitored fraction, which calls a season with episodes still to come 100% complete. The remainder is split into buckets that add back up to the denominator -- **missing** for aired episodes nothing is working on, **downloading** for what is still transferring, **waiting to import** for what has landed and is stuck in front of the import step, and **unaired**/**unmonitored** for the rest -- so an episode already in the queue is never reported as missing. Green is reserved for a season that is genuinely all on disk, and each count is bound to its own words so a narrow phone wraps between phrases, never between "13" and what it counts.
