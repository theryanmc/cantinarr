...
- **Week at a glance** -- the admin issue list opens with the agent scoreboard for the last 7 days: resolved, zero-touch, fixes your standing rules approved, incidents that cleared on their own, what needs you, and paused rules. It sits here rather than on Agent fixes because a quiet week leaves that queue empty, and a scoreboard nobody opens during the quiet weeks cannot be what makes them legible; here the numbers also sit one tab away from the rows they count. Each stat is non-breaking, so a count never splits from the word it counts and the line may only wrap before a separator.
