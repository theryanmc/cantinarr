...
- **Sonarr's own season progress label** -- the arr modules speak arr, so season and All Seasons cards read `11 / 11`, or `0 + 13 / 13` when episodes are on the way: files on disk, plus the queued episodes that have no file yet (queued upgrades add nothing), over the count Sonarr considers obtainable. Colour follows Sonarr's progress-bar kinds -- in flight wins, a full count is green for an ended series and the info tone while it is still running, and a short count is red when monitored, amber when not.
