...
- [ ] `REL-010` · P1 · LIVE — Run Play beta workflow for Android-relevant changes; verify the default upload reaches Open testing (`beta`), the reviewed release is available in the configured countries, and a Google account outside the closed-test email lists can join through the README opt-in link and install it. Verify signed AAB, package/version, passkeys/deep links, explicit `alpha`/`internal` dispatch, build-only PR behavior, and no upload without service-account secret.
