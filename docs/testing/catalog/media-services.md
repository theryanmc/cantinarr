...
- [ ] `BOOK-016` · P1 · UI/LIVE — With no Hardcover token on the Chaptarr instance, verify an admin's Books tab opens with **Trending Books** offering only **Connect Hardcover in Chaptarr settings** (which lands on that instance's editor) and a requester's Books tab shows no such row. Paste a real Hardcover API token in the editor's **Hardcover** section; verify a wrong token is refused as rejected and a valid one reads connected without the token ever appearing in any response. Return to Books: verify the row lists the same top titles hardcover.app/trending shows, in the same order, with covers, that a title the library holds carries its real Available / Partial / Requested pill and one it does not shows its author instead, and that tapping a card opens the same book page a Chaptarr search for that `hc:` id would. Disconnect Hardcover; verify the row returns to the connect prompt on the next load.
