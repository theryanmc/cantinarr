...
Adding a book Chaptarr doesn't already track means finding its metadata record again, and Chaptarr's book lookup is a fuzzy text search — so a title the requester just found in search can still be unfindable a screen later. Cantinarr replays the requester's own search term first (it's the query already proven to return that record, and it's stored on pending rows so approval later uses it too), then falls back to the exact title, id-addressed terms, and the title's headline without its subtitle and trailing parentheticals. Only an exact `foreignBookId` match is ever accepted.
