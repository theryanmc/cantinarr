// Package bookdiscovery serves public book metadata. Library and request
...
var errUnavailable = errors.New("book provider is unavailable; please retry")
...
// A bounded metadata cache also coalesces identical concurrent reads.
	...
	if !exists {
		...
		go func() {
			...
			if err == nil && ttl > 0 {
				...
				for k, e := range m.entries {
					if len(m.entries) < 256 && m.bytes+len(body) <= 32<<20 {
						break
					}
					m.bytes -= len(e.body)
					delete(m.entries, k)
