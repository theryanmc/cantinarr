...
func TestAdminBrowsesBeforeSetupButCannotResolveRequestTarget(t *testing.T) {
	e := newAccessEnv(t)
	for _, id := range []string{e.a, e.b} {
	...
	paths := []string{"/api/discover/books/popular", "/api/genres/book", "/api/media/book/OL1W"}
	// A movies-only server and then a completely fresh server both browse.
	for _, allAbsent := range []bool{false, true} {
		...
		for _, path := range paths {
			...
			if e.hits.Load() != before {
				t.Fatal("denied access reached external provider")
			}
		}
		if w := e.get(5, "admin", "/api/media/book/OL1W/request-target", ""); w.Code != 403 {
			t.Fatalf("request target without instance: %d %s", w.Code, w.Body)
		}
	}
}
