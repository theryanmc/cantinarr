...
GET    /api/admin/agent-digest             # the agent scoreboard: rolling window of resolved (zero-touch and rule-approved are subsets of it), self-cleared, actions executed, tokens; plus needs-a-human/paused counts that are state NOW, not window facts
...
| `agent_digest` | on | admins | the weekly agent scoreboard — the one push that reports success ("Last 7 days: 5 resolved · 3 zero-touch · 2 by your rules. Right now: 2 need you"), skipped entirely on a week with nothing to say |
