...
One shared classifier (`internal/arr/doctor.go`) explains stuck queue items in plain English -- sample files, un-extracted archives, unconfirmed TheXEM mappings, "not an upgrade", unparseable/invalid files, remote-path-mapping or download-client problems, stalled torrents, permissions -- and maps each to ordered one-click fixes: process monitored downloads, manual/force import (candidates shown first, `quality`/`languages` blobs round-tripped verbatim), remove, blocklist + re-search, blocklist without re-search, change category (hand-off to e.g. Unpackerr), rescan. Both blocklist fixes dispatch with `skipRedownload`, so the replacement search is Cantinarr's alone: a blocklisted queue item makes the service fire its own search whenever failed-download handling is enabled, and an approved fix must dispatch exactly one search for one decision — never two, and never an outcome that silently depends on a service setting governing UNATTENDED failures rather than this approval. The blocklist pair is chosen from live library state: when the queue snapshot proves the movie/episode already has a file, the stuck download was only ever an upgrade, so the dead release is dropped and blocklisted with **no** replacement hunt — the copy on disk stays watchable and the service's own RSS pass keeps grabbing better releases as they are posted, which is how such a download arrives in the first place. An automatic replacement search is exactly how a stalled release gets re-grabbed from an indexer listing whose title the blocklist does not match, so it is spent only on a genuine library gap. Unknown file state (Chaptarr never reports one) falls back to re-searching. The same catalog backs the app UI (Sonarr, Radarr, and Chaptarr), the AI assistant, the remediation agent, and the MCP tools; `diagnose_queue` over MCP prints the exact next tool call per item.
...
| `remediate_queue_item` | One-click queue fix: remove, blocklist+search, blocklist without search, change category (admin) |
