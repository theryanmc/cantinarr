...
Adding a brand-new book needs its full metadata record (editions are round-tripped verbatim into the add payload), and Chaptarr's `book/lookup` is a fuzzy text search rather than the id fetch Radarr gets, so the record has to be found again by searching. Chaptarr's own web UI never faces this -- it posts the search row straight back -- but Cantinarr's clients keep only the id and title, so book request bodies carry an optional `search_term`: the text the requester actually searched, which is the one term already proven to return this record. It is tried first and stored on pending rows, so approval days later replays the same starting point instead of guessing. Requests with no search behind them (notification taps, deep links) fall back to the requester's exact title, then id-addressed terms (`edition:`/`work:`/`readarr:`) for forks that support them, then the title's headline with subtitle and trailing parentheticals dropped, which is what rescues the long titles a text search ranks badly. Only an exact `foreignBookId` match is ever accepted, so extra terms can find the right record but never substitute a different one. When no term knows the id there is nothing to add or monitor, so the request is parked as `pending` with an explanatory `message` on the create response instead of being dropped: an admin can add the author in Chaptarr and approve it, or deny it. That parking is specific to unresolved metadata -- a lookup that could not be performed at all, a missing root folder, or ambiguous profiles still fail the request outright, because those are answers the requester needs to see.
